...
--     Excludes patients with pre-existing HF (I50%/428%), cardiomyopathy (I42%/425%),
--     OR arrhythmia/conduction disorder (I44-45%/I47-49%/426-427%) before cohort entry.
--     Uses has_pre_existing_cardiac from file 03. Cleanest drug-attribution; smallest N.
--
...
SELECT
    ...
    COALESCE(pre.has_pre_existing_arrhythmia,   0) AS has_pre_existing_arrhythmia,
    COALESCE(pre.has_pre_existing_hf_or_cmp,    0) AS has_pre_existing_hf_or_cmp,
    COALESCE(pre.has_pre_existing_cardiac,       0) AS has_pre_existing_cardiac,
    ...
    -- ── Eligibility: strict ────────────────────────────────────────────────────
    -- Requires determinable label AND no pre-existing HF, CMP, or arrhythmia AND not already toxic
    CASE
        WHEN ev.first_event_time IS NOT NULL
         AND ev.first_event_time <= e.prediction_time                                              THEN 0
        WHEN COALESCE(pre.has_pre_existing_cardiac, 0) = 1                                        THEN 0
        WHEN ev.first_event_time >  e.prediction_time
...
-- ── Dataset 1: Strict ──────────────────────────────────────────────────────────
-- Excludes patients with pre-existing HF, CMP, or arrhythmia/conduction disorder.
