...
--     428%     — Heart failure
--
-- ── PRE-EXISTING EXCLUSION (strict binary table) — extended ───────────────────
-- Pre-existing HF          : I50% / 428%              → pre_existing_hf
-- Pre-existing CMP         : I42% / 425%              → pre_existing_cardiomyopathy
-- Pre-existing arrhythmia  : I44-45%, I47-49% / 426-427% → pre_existing_arrhythmia
-- Combined flag            : has_pre_existing_cardiac → drives the strict binary table.
--
-- Arrhythmia/conduction codes are added to the pre-existing check because they are
-- chronic conditions (e.g. AF) that recur on every admission. A post-drug admission
-- with I48% is only a new incident event if the patient had no prior AF history.
-- Including them in the pre-existing check prevents re-coded chronic arrhythmias
-- from being mislabeled as drug-induced cardiotoxicity in the strict table.
--
-- Main outputs:
--   hf_diagnoses_all            (HF, CMP, arrhythmia ICD records — pre-existing checks)
...
--   pre_existing_arrhythmia     (arrhythmia/conduction before cohort entry)
--   pre_existing_hf_or_cmp      (combined flag; has_pre_existing_cardiac drives strict table)
--   incident_cv_events          (first broad CV event after cohort entry per patient)

-- ── HF, CMP, and arrhythmia diagnoses (for pre-existing checks only) ──────────
...
WHERE
    (d.icd_version = 10 AND (
        ...
        OR d.icd_code LIKE 'I44%'  -- AV block / conduction disorders
        ...
        OR d.icd_code LIKE 'I49%'  -- Other cardiac arrhythmias
    ))
    OR
    (d.icd_version = 9 AND (
        d.icd_code LIKE '428%'   -- Heart failure
        OR d.icd_code LIKE '425%'  -- Cardiomyopathy
        OR d.icd_code LIKE '426%'  -- Conduction disorders
        OR d.icd_code LIKE '427%'  -- Cardiac arrhythmias (AF, VT, VF)
    ));
...
-- Pre-existing arrhythmia/conduction disorder: I44-45%, I47-49% / 426-427%
-- discharged before first drug.
CREATE OR REPLACE VIEW pre_existing_arrhythmia AS
SELECT DISTINCT
    subject_id,
    1 AS has_pre_existing_arrhythmia
FROM hf_diagnoses_all
WHERE (
    (icd_version = 10 AND (
        icd_code LIKE 'I44%' OR icd_code LIKE 'I45%'
        OR icd_code LIKE 'I47%' OR icd_code LIKE 'I48%' OR icd_code LIKE 'I49%'
    ))
    OR (icd_version = 9 AND (
        icd_code LIKE '426%' OR icd_code LIKE '427%'
    ))
)
AND dischtime IS NOT NULL
AND dischtime <= first_drug_time;

-- Combined pre-existing flag.
-- has_pre_existing_cardiac = HF OR CMP OR arrhythmia — drives the strict binary table.
-- has_pre_existing_hf_or_cmp retained for backward compatibility with pipeline config.
...
SELECT
    ...
    COALESCE(arr.has_pre_existing_arrhythmia,   0) AS has_pre_existing_arrhythmia,
    CASE
    ...
    CASE
        WHEN hf.has_pre_existing_hf            = 1
          OR cmp.has_pre_existing_cmp          = 1
          OR arr.has_pre_existing_arrhythmia   = 1
        THEN 1 ELSE 0
    END AS has_pre_existing_cardiac
...
LEFT JOIN pre_existing_arrhythmia     arr ON p.subject_id = arr.subject_id;
