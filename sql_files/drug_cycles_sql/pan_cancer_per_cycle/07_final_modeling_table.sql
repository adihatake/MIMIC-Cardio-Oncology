-- 07_final_modeling_table.sql  (pan_cancer_per_cycle)
--
...
--
-- Toxicity window per cycle: MAX of class-specific ESC 2022 windows across all
-- drugs administered in that cycle.  For a pure anthracycline cycle: 365d.
-- For a fluoropyrimidine + anthracycline cycle: max(30, 365) = 365d.
--
-- Two binary datasets:
--
--   pcr_final_binary_modeling_table_inclusive
...
--
--   pcr_final_binary_modeling_table_strict
...
--
-- Label values:
--   positive                     : CTRCD event within monitoring window
--   negative_observed            : observed through full window, no CTRCD
...
--
-- Main outputs:
--   pcr_final_cycle_modeling_table
--   pcr_final_binary_modeling_table_inclusive   (written first = canonical)
--   pcr_final_binary_modeling_table_strict

CREATE OR REPLACE VIEW pcr_final_cycle_modeling_table AS
SELECT
    e.subject_id,
    e.cycle_number,
    e.prediction_time,
...
FROM pcr_cycle_exposures                 e
LEFT JOIN pcr_first_combined_cardiotox_event  ev  ON e.subject_id = ev.subject_id
LEFT JOIN pcr_pre_existing_hf_or_cmp         pre ON e.subject_id = pre.subject_id
LEFT JOIN pcr_patient_death                  d   ON e.subject_id = d.subject_id
LEFT JOIN pcr_patient_last_observation       obs ON e.subject_id = obs.subject_id;
...
CREATE OR REPLACE VIEW pcr_final_binary_modeling_table_inclusive AS
SELECT *
FROM pcr_final_cycle_modeling_table
...
CREATE OR REPLACE VIEW pcr_final_binary_modeling_table_strict AS
SELECT *
FROM pcr_final_cycle_modeling_table
