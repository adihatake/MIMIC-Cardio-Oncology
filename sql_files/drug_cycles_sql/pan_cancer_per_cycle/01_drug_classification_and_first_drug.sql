-- 01_drug_classification_and_first_drug.sql  (pan_cancer_per_cycle)
--
...
--
-- Main outputs:
--   pcr_drugs_classified       (all pharmacy rows with drug_class label)
--   pcr_cohort_drug_starts     (target-class rows for cancer patients only)
--   pcr_patient_first_drug     (first drug time per patient — cohort entry anchor)

CREATE OR REPLACE VIEW pcr_drugs_classified AS
SELECT
    subject_id,
    hadm_id,
    pharmacy_id,
...
CREATE OR REPLACE VIEW pcr_cohort_drug_starts AS
SELECT d.*
FROM pcr_drugs_classified d
...
CREATE OR REPLACE VIEW pcr_patient_first_drug AS
SELECT
    subject_id,
