-- 06_observation_and_death.sql  (pan_cancer_uniform_365)
--
...
--
-- Main outputs:
--   pcu_patient_death             (death timestamp per patient)
--   pcu_patient_observation_events (all observation events)
--   pcu_patient_last_observation  (last known observation time per patient)

-- Death timestamps (prefer in-hospital deathtime; fall back to dod midnight)
CREATE OR REPLACE VIEW pcu_patient_death AS
SELECT
    p.subject_id,
    COALESCE(
...
INNER JOIN pcu_patient_first_drug f ON p.subject_id = f.subject_id
...
CREATE OR REPLACE VIEW pcu_patient_observation_events AS
SELECT
    subject_id,
...
WHERE subject_id IN (SELECT subject_id FROM pcu_patient_first_drug)
...
CREATE OR REPLACE VIEW pcu_patient_last_observation AS
SELECT
    subject_id,
    MAX(observation_time) AS last_observation_time
FROM pcu_patient_observation_events
GROUP BY subject_id;
