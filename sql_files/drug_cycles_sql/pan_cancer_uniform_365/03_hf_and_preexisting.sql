-- 03_hf_and_preexisting.sql  (pan_cancer_uniform_365)
--
...
--   Pre-existing HF  : I50% / 428%   → pcu_pre_existing_hf
--   Pre-existing CMP : I42% / 425%   → pcu_pre_existing_cmp
--   Combined flag    : pcu_pre_existing_hf_or_cmp
--
...
--
-- Main outputs:
--   pcu_hf_diagnoses_all          (HF + CMP ICD records for cohort patients)
--   pcu_pre_existing_hf           (HF before cohort entry)
--   pcu_pre_existing_cmp          (CMP before cohort entry)
--   pcu_pre_existing_hf_or_cmp    (combined pre-existing flag)
--   pcu_incident_hf_events        (first HF admission after cohort entry per patient)

-- All HF ICD diagnoses for cohort patients joined to admission timestamps
CREATE OR REPLACE VIEW pcu_hf_diagnoses_all AS
SELECT
    p.subject_id,
    p.first_drug_time,
    a.hadm_id,
...
CREATE OR REPLACE VIEW pcu_pre_existing_hf AS
SELECT DISTINCT
    subject_id,
    1 AS has_pre_existing_hf
FROM pcu_hf_diagnoses_all
...
CREATE OR REPLACE VIEW pcu_pre_existing_cmp AS
SELECT DISTINCT
    subject_id,
...
CREATE OR REPLACE VIEW pcu_pre_existing_hf_or_cmp AS
SELECT
    p.subject_id,
...
FROM (SELECT DISTINCT subject_id FROM pcu_patient_first_drug) p
LEFT JOIN pcu_pre_existing_hf  hf  ON p.subject_id = hf.subject_id
LEFT JOIN pcu_pre_existing_cmp cmp ON p.subject_id = cmp.subject_id;
...
CREATE OR REPLACE VIEW pcu_incident_hf_events AS
SELECT
    subject_id,
