-- 02_cycle_exposures.sql  (pan_cancer_uniform_365)
--
...
--
-- Toxicity window: uniform 365 days from prediction_time for all cycles, regardless
-- of which drug classes are present.  This avoids the label contradictions that arise
-- with class-specific windows when the same event falls inside one cycle's window
-- but outside another's.
--
-- Main output:
--   pcu_cycle_exposures  (one row per subject_id / cycle_number)

-- Aggregate to one row per (patient, date) across all drug classes combined
CREATE OR REPLACE VIEW pcu_exposure_start_days AS
SELECT
    subject_id,
...
CREATE OR REPLACE VIEW pcu_exposure_days_with_gaps AS
SELECT
    d.*,
    LAG(start_date) OVER (
...
FROM pcu_exposure_start_days d;

-- Flag the first day of each new cycle
CREATE OR REPLACE VIEW pcu_exposure_days_with_cycle_flags AS
SELECT
    d.*,
    CASE
        WHEN previous_start_date IS NULL THEN 1
        WHEN days_since_previous_start > (SELECT cycle_gap_days FROM pcu_params) THEN 1
        ELSE 0
    END AS is_new_cycle
FROM pcu_exposure_days_with_gaps d;

-- Assign monotonically-increasing cycle_number per patient
CREATE OR REPLACE VIEW pcu_exposure_days_with_cycle_id AS
SELECT
    d.*,
...
FROM pcu_exposure_days_with_cycle_flags d;

-- One row per (patient, cycle_number) with binary drug-class exposure flags
-- and the uniform 365-day toxicity window applied to all cycles
CREATE OR REPLACE VIEW pcu_cycle_exposures AS
SELECT
    s.subject_id,
    s.cycle_number,
    ...
    -- Uniform window for all cycles
    (SELECT uniform_toxicity_window_days FROM pcu_params) AS toxicity_window_days,
...
FROM pcu_exposure_days_with_cycle_id s
JOIN pcu_cohort_drug_starts d
