-- 04_echo_events.sql  (pan_cancer_uniform_365)
--
...
--
-- Main outputs:
--   pcu_all_lvef                  (all LVEF measurements for cohort patients)
--   pcu_all_gls                   (all GLS measurements for cohort patients)
--   pcu_baseline_lvef             (last pre-drug LVEF within baseline_lookback_days)
--   pcu_baseline_gls              (last pre-drug GLS within baseline_lookback_days)
--   pcu_lvef_cardiotox_events     (LVEF CTRCD events post-drug)
--   pcu_gls_cardiotox_events      (GLS subclinical events post-drug)
--   pcu_first_echo_cardiotox_event (earliest LVEF or GLS event per patient)

-- All LVEF measurements for cohort patients
CREATE OR REPLACE VIEW pcu_all_lvef AS
SELECT
    subject_id,
...
WHERE LOWER(measurement) = 'lvef'
  ...
  AND subject_id IN (SELECT subject_id FROM pcu_patient_first_drug);

-- All GLS measurements for cohort patients (negative convention enforced)
CREATE OR REPLACE VIEW pcu_all_gls AS
SELECT
    subject_id,
    CAST(measurement_datetime AS TIMESTAMP) AS measurement_datetime,
...
CREATE OR REPLACE VIEW pcu_baseline_lvef AS
SELECT
    p.subject_id,
    p.first_drug_time,
    l.measurement_datetime AS baseline_lvef_time,
    l.lvef_value           AS baseline_lvef
FROM pcu_patient_first_drug p
LEFT JOIN pcu_all_lvef l
    ON p.subject_id = l.subject_id
   AND l.measurement_datetime <  p.first_drug_time
   AND l.measurement_datetime >= p.first_drug_time - (SELECT baseline_lookback_days FROM pcu_params) * INTERVAL '1 day'
...
CREATE OR REPLACE VIEW pcu_baseline_gls AS
SELECT
    p.subject_id,
    p.first_drug_time,
...
FROM pcu_patient_first_drug p
LEFT JOIN pcu_all_gls g
    ON p.subject_id = g.subject_id
   AND g.measurement_datetime <  p.first_drug_time
   AND g.measurement_datetime >= p.first_drug_time - (SELECT baseline_lookback_days FROM pcu_params) * INTERVAL '1 day'
QUALIFY ROW_NUMBER() OVER (
    PARTITION BY p.subject_id
...
CREATE OR REPLACE VIEW pcu_lvef_cardiotox_events AS
SELECT
    b.subject_id,
...
CREATE OR REPLACE VIEW pcu_gls_cardiotox_events AS
SELECT
    b.subject_id,
    f.measurement_datetime AS event_time,
...
CREATE OR REPLACE VIEW pcu_first_echo_cardiotox_event AS
SELECT
    subject_id,
...
FROM (
    SELECT * FROM pcu_lvef_cardiotox_events
    UNION ALL
    SELECT * FROM pcu_gls_cardiotox_events
)
QUALIFY ROW_NUMBER() OVER (
